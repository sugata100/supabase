...
- Review table privileges for the `anon` and `authenticated` roles. On many projects, tables in `public` receive broad default grants. Adding policies does not revoke those grants.
  - Check current grants with:
    ...
    ```
  - Prefer a revoke-then-grant pattern so each role keeps only the operations your app needs. See [Enable RLS and set the grants](/docs/guides/database/postgres/row-level-security#enable-rls-and-set-the-grants).
  - Remember that RLS does not cover `TRUNCATE`. Keep that privilege revoked for client roles unless you intentionally need it.
- Enable replication on tables containing sensitive data by enabling RLS and setting row security policies:
  - Go to the [**Database > Policies**](/dashboard/project/_/database/policies) section of the Supabase Dashboard to enable RLS and create security policies.
...
- If your application requires a higher level of security, consider setting up [multi-factor authentication](/docs/guides/auth/auth-mfa) for your users.
...
- Connect your GitHub repository and enable **Deploy to production** in the integration settings to automate deployments from your `main` branch ([set up the integration](/dashboard/project/_/settings/integrations))
  - This ensures schema changes are deployed consistently and avoids manual `supabase db push` from a local machine
- If you're on the Pro Plan, consider enabling [branching](/docs/guides/deployment/branching) to test migrations in a preview environment before they reach production
